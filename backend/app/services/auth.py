...
# logins.
#
# TODO (future Alembic migration): add brute-force lockout columns
# to the users table — failed_login_attempts INT NOT NULL DEFAULT 0
# and locked_until TIMESTAMPTZ — then wire authenticate_user to
# increment on failure, lock at 5, clear on successful workspace
# selection, and silently reject during the lockout window (no
# distinguishable error so the response is still enumeration-safe).
# ================================================================
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User:
    ...
    password, soft-deleted user, inactive user. Distinguishing them
    would leak account state to attackers performing user enumeration.
    """
    ...
    # Always run a bcrypt comparison even on missing user to equalise timing
    if user is None:
    ...
    if not verify_password(password, user.password_hash):
        logger.warning("Login failed: wrong password", extra={"user_id": str(user.id)})
        raise InvalidCredentialsError()
